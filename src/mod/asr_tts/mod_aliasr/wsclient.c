...
{
	const char *key = "dd7d5c1e64c64b81bb65419854a9aa04";
// 	char version[5] = "";
// 	char proto[256] = "";
// 	char proto_buf[384] = "";
	char input[256] = "";
...
//	char respond[512] = "";
	...
	if (ws_raw_write(wsh, request, bytes) != bytes) {
		goto err;
	}
	...
	*(wsh->buffer + wsh->datalen) = '\0';
	...
	if (strncasecmp(wsh->buffer, "HTTP/1.1 ", 9)) {
		goto err;
	}
...
{
	uint8_t hdr[14] = { 0 };
	size_t hlen = 2 + (wsh->use_mask ? 4 : 0);
	uint8_t *bp;
	ssize_t raw_ret = 0;
	...
	if (bytes < 126) {
		hdr[1] = (uint8_t)bytes  | (wsh->use_mask ? 0x80 : 0);
		if (wsh->use_mask) {
			hdr[2] = masKingKey[0];
			hdr[3] = masKingKey[1];
			hdr[4] = masKingKey[2];
